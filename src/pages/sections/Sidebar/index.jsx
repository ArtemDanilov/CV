...
const Sidebar = () => {
  ...
  return (
    <aside className="bg-zinc-700 text-zinc-200 w-full">
      <div className="avatar hidden aspect-square mx-auto p-4 pb-0 md:block">
        <Image
          url={avatar}
          width="300"
          height="300"
        ...
        {sidebar.map(({ id, title, type, items }) => {
          return (
            <Container title={title} key={id}>
              <ul
                className={`${type !== "links" && "list-disc pl-5"} space-y-1`}
              >
                {items.map((item) => (
